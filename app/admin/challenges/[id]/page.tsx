  ...
  if (!challengeData) {
    return (
      <div className="rounded-2xl border border-cafe-border bg-cafe-card p-12 text-center shadow-cafe max-w-lg mx-auto mt-12">
        ...
        <p className="mt-2 text-xs text-cafe-oatmeal">
          The requested challenge ID ({params.id}) does not exist in the database.
        </p>
        <div className="mt-6">
          <Link href="/admin">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-xs text-cafe-oatmeal">
        ...
        <span>/</span>
        <span className="text-cafe-linen truncate max-w-[200px]">
          {challengeData.title}
        </span>
      </div>
